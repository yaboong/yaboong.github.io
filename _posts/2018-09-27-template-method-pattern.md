...
-------------------------------- 여기까지 지난포스팅에서 썼는데 내용을 아래에 추가함 --------------------------------

###### 추가한 내용

인터페이스의 default 제어자에 대해서 구현부를 가질 수 있다는 것에 너무 집착해서 다른 생각을 못했다.
인터페이스의 default 제어자를 가진 메소드는 같은 패키지 내에서만 접근가능하다는 점을 잘 생각해야 한다.

추상클래스에서 접근제어자를 보다 다양하게 사용할 수 있지만 추상클래스로 제공되던 것을 인터페이스로 다 바꿔버리면, 
이 추상클래스가 써드파티 라이브러리로 제공되는 경우 default 로 정의된 메소드의 구현부에 있는 내용을 사용할 방법이 없다.

인터페이스에 default 메소드를 활용하면 구현부를 가질 수 있다고 해서 인터페이스가 추상클래스를 대체할 수 있는 것은 아닌 것 같다!  
