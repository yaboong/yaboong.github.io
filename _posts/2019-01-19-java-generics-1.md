...
&lt;T&gt; 로 표현한 것이 제네릭이다. GenericArrayList 는 객체를 생성할때 타입을 지정하면 생성되는 오브젝트 안에서는 T 의 위치에 지정한 타입이 대체되어서 들어가게 된다.
이 작업은 컴파일 시점에 이루어진다. 아래와 같이 생성하면 컴파일 시점에 T 가 String 으로 대체되어서 컴파일 된다.
...
```java
@Test
@DisplayName("제네릭을 사용하는 ArrayList 테스트")
public void genericArrayListTest() {

    GenericArrayList<Integer> intList = new GenericArrayList<>();
    intList.add(1);

    int intValue = intList.get(0); // 형변환을 하지 않는다

    GenericArrayList<String> stringList = new GenericArrayList<>();
    stringList.add("yaboong.github.io");

    String strValue = stringList.get(0); // 형변환을 하지 않는다
...
또한 잘 구별해야 할 것은, 리턴타입으로 제네릭 타입을 사용하는 것은 제네릭 메서드가 아니라는 것이다.

아래 get() 메서드는 리턴타입을 제네릭으로 지정해주었을 뿐이지, 제네릭 메서드가 아니다.
...
아래 printFirstChar() 메서드는 리턴타입과 상관없이 그 앞에 제네릭 타입에 대한 정의를 해주었다. 이것은 제네릭 메서드가 맞다.

```java
public <T extends CharSequence> void printFirstChar(T param) {
