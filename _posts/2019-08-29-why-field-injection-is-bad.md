...
추측컨데 빈 생성시 아래와 같은 로직이 수행되면서 어떤 시점에 스프링이 그것을 캐치해서 순환참조라고 알려주는 것 같다.
...
수정자 주입을 사용하면 아주 잘 구동되고 순환참조를 하고 있는 부분에 대한 호출이 이루어질 경우 StackOverflowError 를 뱉는다. 적어도 여기서 만큼은 생성자 주입이 Silver Bullet 이라고 할 수 있을 것 같다.
